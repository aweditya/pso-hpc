...
void find_overall_best_fit(particle_t *particles, int num_particles, double *overall_best_fit, int *index_gbest)
{
    ...
    {
        // Compute fitness
        double c = particles[i].position.coordinate[0];
        char c_string[10];
        snprintf(c_string, 10, "%lf", c);

        char alter_cmd[32] = "alter c ";
        char u_cmd[32] = "u\n";
        char cmd[64];
        ...
        strcpy(cmd, "tran 10ms 2s\n");
        ngSpice_Command(cmd);
        state = 3;

        memset(cmd, 0, sizeof(cmd));
        strcpy(cmd, "meas tran teval when v(2)=2.5 cross=last\n");
        ngSpice_Command(cmd);
        state = 1;

        // Find gbest
        current_fitness = (current_fitness - 6.934467e-02) * (current_fitness - 6.934467e-02);
